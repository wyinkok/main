...
respectively. It is inspired by the Instagram Save Collection feature, whereby users are able to personally curate specific
...
private Person addSavedTagToInternship(Person person) throws CommandException {
        ...
        //Adds a new tag savedTagName, which is a String "saved" to the UniqueTagList
        try {
...
----
    private Person removeSavedTagToInternship(Person person) {
        ...
        //Checks for an existing "saved" tag and deletes
...
We chose to use the additional and removal of tags for 2 main reasons.
...
===== Alternative
Another method was to create separate AddressBook Model and its own storage for the Saved Collections
