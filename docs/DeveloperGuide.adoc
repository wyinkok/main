...
In the second phase, the command is being executed in Find Command. The first updateSearchedInternshipList() update the
...
use as the argument in Find Command. Those keywords are then added to the respective internships that matches it through the
addTagsToFilteredList(). Find Command then execute getCommandResult() and return the result of the command execution encapsulated
