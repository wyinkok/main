...
Clears previous message thread with JobbiBot. To restart the conversation, user has to type `start` again except for the `exit` command. +
Format: `new`
...
Filters the internship list according to the given keywords. +
