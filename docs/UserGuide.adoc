...
+
.  Download the latest `jobbibot4.jar` link:{repoURL}/releases[here].
...
=== Starting conversation with program : `start`

Initiates the message thread of JobbiBot. User can still find internships with Jobbi without typing `start` except that user will not get any prompts or responses from Jobbi +
Format: `start`

=== Restarting conversation with program : `new`

Restarts a new message thread of JobbiBot and the previous thread is deleted. User does not have to type `start` to restart the conversation again. +
Format: `new`
...
=== Saving all internships into Saved Collection : `saveall`

Saves all internships in the Saved Collection. +
Format: `saveall`

=== Removing a Saved internship from a Saved Collection : `unsave`
...
=== Removing all saved internships from the Saved Collection : `unsaveall`

Removes all internships in the Saved Collection. +
Format: `unsaveall`

=== Showing the full Saved Internship Collection : `find saved` or `filter saved`
